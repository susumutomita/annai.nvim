...
- 小窓の**タイトルに使用中のバックエンド**（`案内 · afm` / `案内 · Ollama: qwen2.5:3b`）が出る。考え中の本文にもモデル名が出るので、**フォールバック/エスカレーションが効いたか一目で分かる**
- カーソルを動かす / 入力を始めると自動で閉じる

## 答えが違ったら：もう一度 `<leader>?` で詳しく

最初のバックエンド（既定は afm・最速）は、一覧に無い操作だと無理に 1 つ選ぶことがあります。**回答の小窓が出ている間にもう一度 annai を呼ぶ**と、打ち直さずに同じ質問を次のバックエンド（より慎重な Ollama）へ回します。

- 1 回目: `<leader>?`（または `:Annai`）で質問。afm（速い）が即答。違っていそうなら…
- 2 回目（小窓が出たまま `<leader>?` / `:Annai`）: 同じ質問を Ollama が答え直す（`置換` → `この設定には無い` など）
- 小窓は `— 違う？ もう一度 Space ? でじっくり聞く` のように **実際の起動キー** をヒント表示します

> ⚠️ `<leader>?` は **leader（既定はスペース）→ `?`** の 2 キーです。**素の `?` 単体は Vim の検索**になり annai には届きません。キーマップが不安なときは `:Annai`（+ Enter）が確実です。

次のバックエンドが無いとき（afm だけ等）はヒントを出しません。
...
require("annai").setup({
  ...
  stats_top = 5,
  more_hint = "— 違う？ もう一度 %s でじっくり聞く", -- %s に起動キー（Space ? 等）が入る
...
## バックエンドと精度（afm と Ollama）

両バックエンドには性格の違いがあります。

| | afm（Apple オンデバイス） | Ollama |
| --- | --- | --- |
| 速さ | ◎ 最速（~1 秒） | ○ モデル次第（3B で ~2 秒） |
| 一覧に**ある**操作 | ◎ 正しく選ぶ | ◎ 正しく選ぶ |
| 一覧に**ない**操作 | △ **無理に 1 つ選びがち**（例:「置換」→ 無関係なキー） | ◎ 「この設定には無い」と正しく答える |

afm は「該当なし」を返すのが苦手で、プロンプトを強化しても改善しにくい傾向があります（一覧に無い質問でも近そうなキーを 1 つ選んでしまう）。**該当なしの判定まで含めて精度を重視するなら、Ollama を優先**してください。小さい `qwen2.5:3b` でも「この設定には無い」を正しく返します。

```lua
opts = {
  backends = { "ollama", "afm" },        -- Ollama を優先（afm は保険として残す）
  ollama = { model = "qwen2.5:3b" },     -- 速い。さらに賢くしたいなら大きめのモデルへ
}
```

速度重視なら afm 優先（既定）のまま、精度重視なら上記。用途で選べます。
